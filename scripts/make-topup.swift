...
let outDir = CommandLine.arguments.count > 1 ? CommandLine.arguments[1] : "/tmp/stonemux-topup"
...
let gtBig: [P] = [(0.31,0.29),(0.66,0.50),(0.31,0.71)]
let gtSmall: [P] = [(0.35,0.33),(0.62,0.50),(0.35,0.67)]
...
for (si, a) in arts.enumerated() {
    for (sti, _) in styleNames.enumerated() {
        if si == 19 && !(sti == 3 || sti == 4) { continue }
        if !(sti <= 1 || sti >= 3) { continue }   // topup: 只要 wire/knock 四族
        if ![0,1,3,4,10].contains(si) && si != 5 { continue }   // topup: 稳定六石
