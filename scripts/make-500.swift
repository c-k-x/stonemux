...
// 第五弹：500 个候选 = 20 石形 × 5 风格 × 5 配色
// 风格: 0 wireframe 线稿 | 1 facets 切面填充 | 2 neon 霓虹辉光 | 3 pixel 像素 | 4 holo 全息扫描线
// 配色: 0 终端绿 | 1 赛博青 | 2 星云紫 | 3 琥珀 | 4 优雅金
// 用法: swift scripts/make-500.swift <输出目录>   （后台跑，不打开 html）
...
func allPolys(_ a: Art) -> [[P]] {
    var out = a.sil
    for o in a.ovals { out.append(ovalPoly(o)) }
    return out
}

func draw(_ a: Art, style: Int, pal: (RGB, RGB, RGB), size: Int) -> NSImage {
    let S = CGFloat(size)
    ...
    case 0: // wireframe
        ...
        for line in a.inner { let p = path(line, close: false); p.lineWidth = lw; p.stroke() }
    case 1: // facet fill + edges
        for poly in sils {
            let p = path(poly, close: true)
            color(pal.2, 0.16).setFill(); p.fill()
        }
        color(pal.1).setStroke()
        for poly in sils { let p = path(poly, close: true); p.lineWidth = lw * 0.8; p.stroke() }
        color(pal.2).setStroke()
        for line in a.inner { let p = path(line, close: false); p.lineWidth = lw * 0.8; p.stroke() }
    case 2: // neon glow
        for (w, al) in [(lw * 3.2, 0.10), (lw * 1.8, 0.22), (lw * 0.8, 1.0)] {
            color(pal.2, al).setStroke()
            for poly in sils { let p = path(poly, close: true); p.lineWidth = w; p.stroke() }
            for line in a.inner { let p = path(line, close: false); p.lineWidth = w; p.stroke() }
        }
    case 3: // pixel
        let grid = S / 34
        var segs: [[P]] = []
        for poly in sils { segs.append(poly + [poly[0]]) }
        segs += a.inner
        color(pal.1).setFill()
        for seg in segs {
            for i in 0..<seg.count - 1 {
                let (x0, y0) = (pt(seg[i]).x, pt(seg[i]).y)
                let (x1, y1) = (pt(seg[i+1]).x, pt(seg[i+1]).y)
                let len = max(abs(x1 - x0), abs(y1 - y0))
                let steps = max(1, Int(len / grid))
                for s in 0...steps {
                    let t = CGFloat(s) / CGFloat(steps)
                    let px = (x0 + (x1 - x0) * t) / grid
                    let py = (y0 + (y1 - y0) * t) / grid
                    let gx = floor(px) * grid, gy = floor(py) * grid
                    NSRect(x: gx + grid * 0.1, y: gy + grid * 0.1,
                           width: grid * 0.8, height: grid * 0.8).fill()
                }
            }
        }
    default: // holo scanlines
        NSGraphicsContext.saveGraphicsState()
        let clip = NSBezierPath()
        for poly in sils { clip.append(path(poly, close: true)) }
        clip.addClip()
        color(pal.1, 0.45).setFill()
        var y: CGFloat = 0
        while y < S {
            NSRect(x: 0, y: y, width: S, height: S * 0.008).fill()
            y += S * 0.035
        }
        NSGraphicsContext.restoreGraphicsState()
        color(pal.2).setStroke()
        for poly in sils { let p = path(poly, close: true); p.lineWidth = lw * 0.7; p.stroke() }
    }
    img.unlockFocus()
    return img
}
...
for (si, a) in arts.enumerated() {
    for (sti, _) in styleNames.enumerated() {
        for (pi, _) in palettes.enumerated() {
            ...
            png(draw(a, style: sti, pal: palettes[pi], size: 512), "\(outDir)/\(code).png")
...
let html = """
<html><head><meta charset="utf-8"><title>stonemux 500 候选</title></head>
<body style="background:#151517;color:#eee;font-family:-apple-system,sans-serif;padding:24px">
<h1>500 候选（编号 = 石形×风格×配色）</h1>
<p style="color:#888">风格: wire 线稿 / facet 切面 / neon 霓虹 / pixel 像素 / holo 全息 · 配色 p0 终端绿 p1 赛博青 p2 星云紫 p3 琥珀 p4 优雅金</p>
...
print("done: \(n) icons -> \(outDir)/gallery.html")
